...
| 40 | `fix/an-oversize-body-carries-a-problem` | A body over the limit answers `413` with a `ProblemDetail` and a new `ProblemCode.BODY_TOO_LARGE`, a failing test first. Candidate: a Vert.x route ordered before the body handler, refusing on `Content-Length`. If no mechanism covers it, the teammate stops and asks (tier 2). The two upload routes declare the code. (Corrected: a chunked body, which carries no Content-Length, is cut by RESTEasy Reactive's own bodyless `413` and stays so, the operator having accepted it on 2026-09-23: each upload route's `413` names the limit it publishes and says so. `BodyLimitCheck` refuses the boot when `images.max_file_bytes` or `imports.max_chunk_bytes` is not strictly under the body limit, replacing that case of `ImportsConfigIntegrationTest` and the trap comment in `application.properties`.) |
