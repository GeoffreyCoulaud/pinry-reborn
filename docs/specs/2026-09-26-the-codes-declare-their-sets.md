...
lost transform leaves unused, TS2578.
...
| 20 | `feat/the-open-codes-are-published` | Mapper tests, as `PinImageStateMapperTest` is written, covering every entry of `DownloadReason` and `UserDataImportIssueKind`. A contract test: `DownloadReasonDto` and `UserDataImportIssueKindDto` carry `x-extensible-enum` equal to the twin's names and no `enum`, and the four positions reference them. `dagger call contract-guard` green at `19.0.0`, red at `18.1.0`. `downloadReasons.ts` and `importIssues.ts` typed by `Known<...>`, their comments rewritten, and the committed guard of decision D, which generating through the command line fails. `agents/engineering.md`'s "This project's API contract" names `x-extensible-enum` and `clients/AGENTS.md` names the `transform` (decision H) |
