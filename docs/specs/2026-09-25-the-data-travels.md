...
with the journey, the section arriving in the next block. *(Corrected: the whole block measured
892 lines over 17 files against `feat/the-export-is-reachable`, and this seam left the section's
side at about 593. The operator answered "b" on 2026-09-25: three blocks, 30 the store and
`lib/imports.ts`, 34 the section's read side with the cancel confirmation and the journey's cancel
case, 37 the file picker, the upload view and the journey's upload cases.)* Each block measures
its budget once committed, against its parent branch (section 8).
