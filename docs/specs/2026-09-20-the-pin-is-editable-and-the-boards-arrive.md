...
| M | The image has two controls and no delete: replace it with a file, and fetch it again from `sourceMediaUrl`, the second absent when the pin holds no address. Both are one `PUT /api/v1/pins/{pinId}/image`, which is `useSetPinImage` as it stands (Corrected in block 70, the operator's review of 2026-09-20: the form offers a choice between three options rather than two controls that act on their own. **Keep**, the default; **replace**, over the creation dialog's own drop area; and **fetch from URL**, offered as soon as the address field holds one rather than only when the pin was read with one. Nothing reaches the server until Save, which writes the pin first and then applies the chosen option, so a fetch uses the address the pin now holds. The edit view also shows the image, which it did not.) | Property 8: the route already supersedes, atomically, and the old image stands until the new one lands. Having no delete control is also what keeps the rule that a pin is given either an upload or an address, with no intermediate state to validate. The correction's reason is the same rule read one level up: two controls acting immediately let a user fetch from an address the pin write had not saved, or cancel an edit whose image had already changed, which is the intermediate state the decision exists to refuse |
...
2026-09-20.** Measured at its tip, `git diff --numstat` against `main` in pull request #172: 615
lines total against a strict 600, and 486 production lines under `clients/` against a strict 400.
Of those 486, 357 are one move: `components/PinGrid.tsx` created at 181 lines and `routes/Home.tsx`
shortened by 176 of the same code, which `git diff --numstat` counts on both sides. A split was
offered, the extraction alone against the board's screen, which that arithmetic puts at about 360
and 255 lines; the operator declined it because the budget measures what a human rereads and a
reader of this block rereads the move once. Nothing here was trimmed to fit, there being no padding
to trim. (The figures the operator decided on were 609 and 480, read before the browser pass; the
fix it produced is the difference.)
