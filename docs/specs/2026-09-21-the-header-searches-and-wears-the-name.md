...
| 20 | `feat/the-pin-search-route-goes` | `GET /api/v1/pins/search` answers 404, which discriminates: a controller still registered answers 400 on a missing `q`, the literal path winning over `@Path("/{pinId}")`; the contract holds no such path; `oasdiff` accepts `8.0.0` |
