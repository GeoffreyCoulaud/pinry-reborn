...
| Path          | Role                                                                                           |
|---------------|-------------------------------------------------------------------------------------------------|
| `lib/`        | **Pure functions only**: no I/O, no DOM, no ambient present. A deterministic global is allowed, `btoa` and `TextEncoder` being the ones already there. This is the whole of the coverage perimeter. |
...
- `pnpm install` : the whole workspace, and the typed client with it. The gate uses `--frozen-lockfile`, so a
  manifest changed without a matching lockfile fails there rather than resolving quietly.
- `pnpm --filter @pinry-reborn/api-client run generate` : rewrites `packages/api-client/src/schema.d.ts` from
  `contract/openapi.json`, through `packages/api-client/generate.mjs` and `openapi-typescript`'s Node API.
  **Run it after any contract change**, the gotcha below saying why an install may not.
...
- **`packages/api-client/src/schema.d.ts` is generated by a `prepare` script and not committed** (ADR 0024,
...
- **An `x-extensible-enum` field is a `string`, and its known values are `Known<"Component">`**, read from
  the `extensibleEnums` interface `generate.mjs` appends. The usual `"A" | (string & {})` does not survive
  openapi-fetch's `Readable`, which maps it as an object. Key a sentence table by `Known<...>`, so a value the
  server adds fails `tsc`, and keep a fallback for the value a newer server sends.
...
- **MSW starts at the top of `src/test/setup.ts`, not in `beforeAll`.** `openapi-fetch` reads `globalThis.fetch`
  ...
  and before any hook. Started late, every journey reaches the real network and reads whatever answers on
  `localhost:3000`.
