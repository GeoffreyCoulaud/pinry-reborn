...
- **`JAVA_HOME` on JDK 25**, exported before any `./gradlew`:
  ...
  paths in the variable. A shell that falls back to JDK 21 fails detekt on `class file version 69`. The toolchain the
  build compiles against is provisioned automatically; the JVM Gradle itself runs on is not. `pre-commit` runs no
  Gradle, only the dash check; `pre-push` runs the gate, through Dagger.
